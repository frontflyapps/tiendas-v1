import { Component, HostListener, OnInit, Output, ViewEncapsulation, EventEmitter } from '@angular/core';
...
@Component({
  selector: 'app-my-account-tcp',
  templateUrl: './my-account-tcp.component.html',
  styleUrls: ['./my-account-tcp.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class MyAccountTcpComponent implements OnInit {
  innerWidth: any;
  passType = 'password';
  passType2 = 'password';
  applyStyle = false;
  ...
  userCodeControl: FormControl = new FormControl(null);
  ...
  ngOnInit() {
    ...
    this.userCodeControl.valueChanges.subscribe((userCodeStatus) => {
      this.setValidationToControl(this.registrationForm.get('userCode'), userCodeStatus);
    });
  }

  setValidationToControl(control: AbstractControl | FormControl, status: boolean) {
    if (status) {
  ...
  // card26: req.body.bankAccountCard26,
  // bankAccountCard26: req.body.bankAccountCard26,
  // bankNameCard26: req.body.bankNameCard26,
  // bankOfficeCard26: req.body.bankOfficeCard26,
  ...
  onSignUp() {
    ...
    data.role = 'Client';
    data.file = this.pdfData[0].file.data;
    ...
    this.authService.singUp(data).subscribe(
      () => {
        this.toastr.showSucces(
          this.translate.instant(
            `Application created successfully, you will receive an email with the response to your registration request.`,
          ),
