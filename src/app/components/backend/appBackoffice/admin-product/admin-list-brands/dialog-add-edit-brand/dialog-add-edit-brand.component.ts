  ...
  createForm(): void {
    if (this.isEditing) {
      this.form = this.fb.group({
        name: [
          this.selectedBrand && this.selectedBrand.name ? this.selectedBrand.name[this.language] : null,
          [Validators.minLength(5), Validators.pattern(/^\w((?!\s{2}).)*/), Validators.required],
        ],
        description: [
          this.selectedBrand && this.selectedBrand.description ? this.selectedBrand.description[this.language] : null,
        ],
        model: [this.selectedBrand && this.selectedBrand.model ? this.selectedBrand.model[this.language] : null],
      });
      ...
      this.form = this.fb.group({
        name: [null, [Validators.minLength(5), Validators.pattern(/^\w((?!\s{2}).)*/), Validators.required]],
        description: [null],
        model: [null],
      });
    }
  ...
  onSave(): void {
    ...
    if (!this.isEditing) {
      ...
      this.categoriesService.createBrand(data).subscribe(
        () => {
          this.showSnackbar.showSucces(this.translate.instant('Brand successfully created'));
          this.spinner.hide();
      ...
      data = this.parseLanguajeEdit(data, this.selectedBrand, this.language);
      data.id = this.selectedBrand.id;
      console.log(data);
      this.categoriesService.editBrand(data).subscribe(
        () => {
          this.showSnackbar.showSucces(this.translate.instant('Updated brand successfully'));
          this.spinner.hide();
          this.dialogRef.close(true);
        },
        (error) => {
