...
@Component({
  selector: 'app-orders-recieved-table',
  templateUrl: './orders-recieved-table.component.html',
  styleUrls: ['./orders-recieved-table.component.scss'],
  animations: [
...
export class OrdersRecievedTableComponent implements OnInit, OnDestroy {
  urlImage: string;
  status = 'delivered';
  allDeliveries: any[] = [];
  columnsToDisplay = [
  ...
  refreshData(): void {
    ...
    if (searchvalue && searchvalue !== '') {
      ...
      this.query.filter.properties.push('filter[$or][Payment][City][name][$like]');
      this.query.filter.properties.push('filter[$or][Payment][Country][name][$like]');
